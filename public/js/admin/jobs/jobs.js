$(document).ready(function () {
    ...
    var table = $("#show-job-data").DataTable({
        ...
        dom: '<"toolbar">Blfrtip' // ✅ only ONE dom key
    });
    ...
    function clearModal() {
        ...
        $(".form")[0].reset(); // reset form safely
    }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.message.image) {
                    $("#jobImage").html(
                        `<img src="/uploads/${response.message.image}" alt="Job Image" width="100" height="100" onerror="this.src='/defaultimage/defaultimage.webp';">`
                    );
                }
                if (response.message.pdf) {
                    $("#jobPdf").html(
                        `<a href="/uploads/${response.message.pdf}" target="_blank" class="btn btn-sm btn-info">View PDF</a>`
                    );
                }
            }
        });
